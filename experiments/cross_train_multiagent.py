...
def main(args):
    ...
    def make_env():
        if args.multiagent:
            ...
            print("Creating multi-agent environment with parameter sharing...")
    ...
    print(f"Starting {args.algorithm.upper()} training in {mode_str} mode...")
    ...
    try:
        ...
        if args.normalize:
            ...
            if args.multiagent:
                norm_name = f"{norm_name.replace('.pkl', '')}_multiagent.pkl"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train traffic light controller with various RL algorithms from Stable Baselines3 (single or multi-agent)"
    )
